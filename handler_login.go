...
// Sử dụng map[string]interface{} để trả về full trường
...
func xu_ly_lay_du_lieu(sid, deviceId string, body map[string]interface{}, action string) (*LoginResponse, error) {
	...
	// ❌ ĐÃ XÓA BƯỚC B (CHECK ASSIGNED MAP) VÌ GÂY SAI THỨ TỰ ƯU TIÊN
	...
	// C. UNIFIED PRIORITY LOOP (Vòng lặp chuẩn Node.js)
	if action != "view_only" {
		isReset := false
			...
			for _, idx := range indices {
				if idx < rawLen {
					...
					if (step.IsMy && isMyNick) || (step.IsEmpty && isEmptyNick) {
						...
						STATE.SheetMutex.Lock()    // Bắt đầu khóa ghi
						...
						STATE.SheetMutex.Unlock()
						STATE.SheetMutex.RLock() // Khóa đọc lại để tiếp tục
					}
				}
			}
		}
