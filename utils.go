...
func ToSlice(v interface{}) []string {
	...
	if strVal, ok := v.(string); ok && strVal == "" { return []string{""} } // Giữ empty string nếu cần
	return []string{}
}
...
type QualityResult struct {
	Valid bool; SystemEmail string; Missing string
}
...
// =================================================================================================
// 🔥 BỘ MÁY LỌC (FILTER ENGINE) - DÙNG CHUNG CHO LOGIN & UPDATE
// =================================================================================================

type CriteriaSet struct {
	MatchCols    map[int][]string; ContainsCols map[int][]string
	MinCols      map[int]float64; MaxCols      map[int]float64; TimeCols     map[int]float64
	IsEmpty      bool
}

type FilterParams struct {
	AndCriteria CriteriaSet; OrCriteria  CriteriaSet; HasFilter   bool
}
...
func parseCriteriaSet(input interface{}) CriteriaSet {
	...
	for k, v := range data {
		if strings.HasPrefix(k, "match_col_") {
			if idx, err := strconv.Atoi(strings.TrimPrefix(k, "match_col_")); err == nil { c.MatchCols[idx] = ToSlice(v); c.IsEmpty = false }
		} else if strings.HasPrefix(k, "contains_col_") {
			if idx, err := strconv.Atoi(strings.TrimPrefix(k, "contains_col_")); err == nil { c.ContainsCols[idx] = ToSlice(v); c.IsEmpty = false }
		} else if strings.HasPrefix(k, "min_col_") {
			if idx, err := strconv.Atoi(strings.TrimPrefix(k, "min_col_")); err == nil { if val, ok := toFloat(v); ok { c.MinCols[idx] = val; c.IsEmpty = false } }
		} else if strings.HasPrefix(k, "max_col_") {
			if idx, err := strconv.Atoi(strings.TrimPrefix(k, "max_col_")); err == nil { if val, ok := toFloat(v); ok { c.MaxCols[idx] = val; c.IsEmpty = false } }
		} else if strings.HasPrefix(k, "last_hours_col_") {
			if idx, err := strconv.Atoi(strings.TrimPrefix(k, "last_hours_col_")); err == nil { if val, ok := toFloat(v); ok { c.TimeCols[idx] = val; c.IsEmpty = false } }
		}
	}
	return c
}
...
func parseFilterParams(body map[string]interface{}) FilterParams {
	...
	f.AndCriteria = parseCriteriaSet(body) // Parse root level
	if v, ok := body["and"]; ok {
	...
	if v, ok := body["or"]; ok { f.OrCriteria = parseCriteriaSet(v) }
...
func isRowMatched(cleanRow []string, rawRow []interface{}, f FilterParams) bool {
	if !f.AndCriteria.IsEmpty { if !checkCriteriaMatch(cleanRow, rawRow, f.AndCriteria, true) { return false } }
	if !f.OrCriteria.IsEmpty { if !checkCriteriaMatch(cleanRow, rawRow, f.OrCriteria, false) { return false } }
	return true
}

// --- 3. PROFILE MAKERS (STRUCTS) ---
// (Giữ nguyên các hàm MakeAuthProfile, MakeActivityProfile... như cũ)
type AuthProfile struct {
	Status string `json:"status"`; Note string `json:"note"`; DeviceId string `json:"device_id"`
	UserId string `json:"user_id"`; UserSec string `json:"user_sec"`; UserName string `json:"user_name"`
	Email string `json:"email"`; NickName string `json:"nick_name"`; Password string `json:"password"`
	PasswordEmail string `json:"password_email"`; RecoveryEmail string `json:"recovery_email"`; TwoFa string `json:"two_fa"`
	Phone string `json:"phone"`; Birthday string `json:"birthday"`; ClientId string `json:"client_id"`
	RefreshToken string `json:"refresh_token"`; AccessToken string `json:"access_token"`; Cookie string `json:"cookie"`
	UserAgent string `json:"user_agent"`; Proxy string `json:"proxy"`; ProxyExpired string `json:"proxy_expired"`
	CreateCountry string `json:"create_country"`; CreateTime string `json:"create_time"`
}

type ActivityProfile struct {
	StatusPost string `json:"status_post"`; DailyPostLimit string `json:"daily_post_limit"`; TodayPostCount string `json:"today_post_count"`
	DailyFollowLimit string `json:"daily_follow_limit"`; TodayFollowCount string `json:"today_follow_count"`; LastActiveDate string `json:"last_active_date"`
	FollowerCount string `json:"follower_count"`; FollowingCount string `json:"following_count"`; LikesCount string `json:"likes_count"`
	VideoCount string `json:"video_count"`; StatusLive string `json:"status_live"`; LivePhoneAccess string `json:"live_phone_access"`
	LiveStudioAccess string `json:"live_studio_access"`; LiveKey string `json:"live_key"`; LastLiveDuration string `json:"last_live_duration"`
	ShopRole string `json:"shop_role"`; ShopId string `json:"shop_id"`; ProductCount string `json:"product_count"`; ShopHealth string `json:"shop_health"`
	TotalOrders string `json:"total_orders"`; TotalRevenue string `json:"total_revenue"`; CommissionRate string `json:"commission_rate"`
}

type AiProfile struct {
	Signature string `json:"signature"`; DefaultCategory string `json:"default_category"`; DefaultProduct string `json:"default_product"`
	PreferredKeywords string `json:"preferred_keywords"`; PreferredHashtags string `json:"preferred_hashtags"`; WritingStyle string `json:"writing_style"`
	MainGoal string `json:"main_goal"`; DefaultCta string `json:"default_cta"`; ContentLength string `json:"content_length"`
	ContentType string `json:"content_type"`; TargetAudience string `json:"target_audience"`; VisualStyle string `json:"visual_style"`
	AiPersona string `json:"ai_persona"`; BannedKeywords string `json:"banned_keywords"`; ContentLanguage string `json:"content_language"`; Country string `json:"country"`
}
